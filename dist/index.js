...
registerExtension({
    name: 'triage',
    ipcHandlers: {
        queue_swe_task: (data, sourceGroup, isMain, _deps) => handleQueueSweTask(data, sourceGroup, isMain),
        set_github_issue: (data, _sourceGroup, _isMain, deps) => handleSetGithubIssue(data, deps),
    },
    onStartup: (deps) => {
        startSweQueueLoop({
            sendMessage: (jid, text) => deps.router.send(jid, text),
            runSweAgent: async (task) => {
                deps.logger.info({ taskId: task.id }, 'SWE agent placeholder — not yet implemented');
                return {};
            },
        });
    },
