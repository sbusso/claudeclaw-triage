...
describe('handleQueueSweTask', () => {
    ...
    it('inserts task from slack_ group', async () => {
        ...
        }, 'slack_nanodev_thread_123', false);
        expect(mockInsert).toHaveBeenCalledWith(expect.objectContaining({ record_id: 'Rec123', task_type: 'fix' }));
    });
    ...
    it('rejects with missing fields', async () => {
        await handleQueueSweTask({ recordId: 'Rec123' }, 'slack_nanodev_thread_123', false);
        expect(mockInsert).not.toHaveBeenCalled();
    });
});
describe('handleSetGithubIssue', () => {
    ...
    it('posts to dev channel', async () => {
        ...
        expect(deps.sendMessage).toHaveBeenCalledWith('slack:CDEV', expect.stringContaining('Bug: broken login'));
    });
